...
st.divider(); st.markdown("### Semáforo de programación")
s1,s2,s3,s4,s5=st.columns(5)
s1.metric("🟢 Al día",resumen["vigentes"]); s2.metric("🟡 Próximas",resumen["proximas"]); s3.metric("🔴 Vencidas",resumen["vencidas"]); s4.metric("🔵 Sin verificar",resumen["sin_verificar"]); s5.metric("⚪ Sin frecuencia",resumen["sin_frecuencia"])
if agenda.empty: st.success("No hay equipos vencidos, próximos o sin verificar.")
else: st.dataframe(agenda,width="stretch",hide_index=True,height=min(430,38+len(agenda)*35))
